...
describe('Channel', () => {
    ...
    it('selectPut and take works on unbuffered channel', () => {
        const ch = new Channel<string>(0);
        const putSpec: PutOperation<string> = {
            ch,
            op: OperationType.PUT,
            value: 'foo'
        };
        let putCbCalled = false;
        const putCb = (err: any, spec: PutOperation<string>) => {
            putCbCalled = true;
    ...
    it('selectTake and put works on unbuffered channel', () => {
        const ch = new Channel<string>(0);
        const takeSpec: TakeOperation<string> = {
            ch,
        ...
        const takeCb = (err: any, spec: TakeOperation<string>) => {
            takeCbCalled = true;
    ...
    it('If a select take is available, I can always selectPutSync, even if it causes overflow', () => {
        ...
        const takeSpec: TakeManyOperation<string> = {
            ch,
    ...
    it('If two selectTakes are pending, and items are only present for one, it will apply the correct one', () => {
        const ch = new Channel<string>(0);
        const takeOneSpec: TakeOperation<string> = {
            ch,
            op: OperationType.TAKE
        };
        const takeTwoSpec: TakeManyOperation<string> = {
            ch,
            op: OperationType.TAKE_MANY,
            count: 2
        };
        let cbCalled = false;
        const cb = (err: any, spec: Operation<string>) => {
            ch._unselect(spec);
    ...
    it('selectTake is applied when a channel is closed', () => {
        ...
        let cbCalled = false;
        const cb = (err: any, spec: TakeOperation<string>) => {
            cbCalled = true;
            ch._unselect(spec);
    ...
    it('A select put many is applied when possible', () => {
        const ch = new Channel<string>(1);
        const putManySpec: PutManyOperation<string> = {
            op: OperationType.PUT_MANY,
            ch,
            values: ['foo', 'bar']
        };
        let cbCalled = false;
        const cb = (err: any, spec: PutManyOperation<string>) => {
            cbCalled = true;
